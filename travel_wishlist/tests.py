...
class TestVisitedList(TestCase):
    ...
    def test_wishlist_contains_visited_places(self):
...
class TestAddNewPlace(TestCase):
    def test_add_new_unvisited_place(self):
        add_pace_url = reverse('place_list')
        new_place_data = {'name': 'Tokyo', 'visited': False}
        response = self.client.post(add_pace_url, new_place_data, follow=True)
...
class TestVisitedPlace(TestCase):
    ...
    def test_non_existent_place(self):
        visit_nonexistent_place_url = reverse('place_was_visited', args=(1000, ))
